...
model_small = load_model('./my_model_small.h5', compile=False)
model_medium = load_model('./my_model_medium.h5', compile=False)
model_dropout = load_model('./my_model_w_dropout.h5', compile=False)

model_small.compile(optimizer=Adam(lr=0.001),loss='categorical_crossentropy',metrics=['accuracy'])
model_medium.compile(optimizer=Adam(lr=0.001),loss='categorical_crossentropy',metrics=['accuracy'])
model_dropout.compile(optimizer=Adam(lr=0.001),loss='categorical_crossentropy',metrics=['accuracy'])
...
t2 = threading.Thread(target=classify_tf, args=(model_dropout, frame_queue2, o2), daemon=True)
t3 = threading.Thread(target=classify_tf, args=(model_medium, frame_queue3, o3), daemon=True)
t4 = threading.Thread(target=classify_tf, args=(model_small, frame_queue4, o4), daemon=True)
